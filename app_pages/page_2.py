...
def render(df):
    ...
    st.markdown("**Business Requirement 1**: Analyze loyalty patterns to identify key churn drivers")
    ...
    if st.checkbox("Show Detailed EDA", value=True):
        ...
        **Interpretation**: Two-year contracts show **{contract_churn['Month-to-month'] - contract_churn['Two year']:.1f}%** 
        lower churn rate compared to month-to-month contracts, strongly validating our hypothesis that 
        ...
        **Key Insight**: Fiber optic users have significantly higher churn rates than DSL users, 
        ...
        if service_results:
            ...
            fig_services = go.Figure(data=[
                go.Bar(name='With Service', x=service_df['Service'], y=service_df['With_Service']),
                go.Bar(name='Without Service', x=service_df['Service'], y=service_df['Without_Service'])
            ])
