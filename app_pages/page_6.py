...
def render(df, ltv_model):
    ...
    **Decision Rules**:
    ...
    fig_roi = px.bar(
        ...
        labels={'Expected_ROI': 'Expected ROI ($)', 'Cluster_Name': 'Customer Segment'},
        color='Expected_ROI',
    ...
    st.success(f"""
    ✅ **Key Insights**:
    - **Highest ROI Cluster**: {highest_roi_cluster['Cluster_Name']} (${highest_roi_cluster['Expected_ROI']:.0f} per customer)
    - **Total ROI Potential**: ${total_potential:,.0f} across all at-risk customers
    - **High Priority Segments**: {len(df_clusters[df_clusters['Priority'] == 'High'])} clusters
    ...
    **Business Impact**: 
    ...
    # Budget table
    budget_df = df_clusters[['Cluster_Name', 'Customer_Count', 'Expected_ROI', 'Recommended_Budget', 'Priority']].copy()
    ...
    st.success(f"""
    ✅ **Budget Optimization**:
    - **Expected Total ROI**: ${df_clusters['Total_ROI_Potential'].sum():,.0f}
    - **Campaign Investment**: $100,000
    - **Net Profit**: ${df_clusters['Total_ROI_Potential'].sum() - 100000:,.0f}
    - **ROI Multiplier**: {df_clusters['Total_ROI_Potential'].sum() / 100000:.2f}x
    
    For every $1 invested in retention, expect **${df_clusters['Total_ROI_Potential'].sum() / 100000:.2f}** in return.
    """)
    ...
    **With ML-Driven Retention**:
    ...
    - Revenue protection: ${revenue_loss * 0.125:,.0f} annually
    - Campaign ROI: {df_clusters['Total_ROI_Potential'].sum() / 100000:.1f}x
