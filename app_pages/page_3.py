...
def render(df, churn_model, ltv_model, cluster_model):
    ...
    st.markdown(
        "**Business Requirements 2-5**: Predict churn risk, customer segment, retention recommendations, and LTV")
    ...
    st.markdown(
        "Enter customer information to get predictions and recommendations:")
    ...
    with col2:
        ...
        phone_service = st.selectbox(
            "Phone Service", df['PhoneService'].unique())
    ...
    with col3:
        ...
        paperless_billing = st.selectbox(
            "Paperless Billing", df['PaperlessBilling'].unique())
        payment_method = st.selectbox(
            "Payment Method", df['PaymentMethod'].unique())
        monthly_charges = st.slider(
            "Monthly Charges ($)", 18.0, 120.0, 70.0, 0.5)
    ...
    if st.button("🚀 Run Predictive Analysis", type="primary", width='stretch'):
        ...
        try:
            # Prepare features for CHURN model (includes tenure and
            # TotalCharges)
            ...
            # Prepare features for CLUSTER model (excludes TotalCharges but
            # includes tenure)
            ...
            with col1:
                ...
                st.caption(f"Predicted churn: {churn_prediction}")

            with col2:
                st.metric(
                    "Predicted LTV", f"${
                        ltv_value:.2f}", delta=f"{
                        'High' if ltv_value > 2000 else 'Medium' if ltv_value > 1000 else 'Low'} Value")
            ...
            with col4:
                ...
                success_rate = 0.3
                expected_roi = max(
                    0, (churn_risk * ltv_value * success_rate) - intervention_cost)
                st.metric(
                    "Retention ROI",
                    f"${expected_roi:.2f}",
            ...
            **Model Performance**: This represents a significant improvement over random guessing (50% accuracy)
            ...
            if churn_risk > 0.7:
                if ltv_value > 2000:
                    recommendation = (
                        "**HIGH PRIORITY**: Immediate personal contact + 20% "
                        "discount offer"
                    )
                elif ltv_value > 1000:
                    recommendation = (
                        "**MEDIUM PRIORITY**: Call within 48h + service "
                        "upgrade offer"
                    )
                else:
                    recommendation = (
                        "**LOW PRIORITY**: Automated email campaign + "
                        "basic discount"
                    )
            elif churn_risk > 0.4:
                if ltv_value > 2000:
                    recommendation = (
                        "**WATCH LIST**: Proactive engagement + loyalty "
                        "rewards"
                    )
                else:
            ...
            try:
                ...
                if isinstance(shap_values, list):
                    ...
                    shap_values_plot = shap_values[1][0]
                else:
            ...
            cluster_profiles = {
                0: {
                    "name": "Young Tech-Savvy",
                    "description": "Month-to-month, high internet usage",
                    "churn_risk": "Medium",
                },
                1: {
                    "name": "Senior Basic Service",
                    "description": "Limited services, price-sensitive",
                    "churn_risk": "High",
                },
                2: {
                    "name": "Family Bundle",
                    "description": "Multiple services, moderate tenure",
                    "churn_risk": "Medium",
                },
                3: {
                    "name": "Long-term Loyal",
                    "description": "Two-year contracts, low churn",
                    "churn_risk": "Low",
                },
                4: {
                    "name": "Premium Fiber Users",
                    "description": "High monthly charges, fiber optic",
                    "churn_risk": "Medium",
                },
                5: {
                    "name": "Budget Conscious",
                    "description": "Basic phone only, high churn",
                    "churn_risk": "High",
                },
                6: {
                    "name": "New Customers",
                    "description": "Low tenure, evaluating service",
                    "churn_risk": "Medium",
                },
            }
            ...
            **Typical Churn Risk**: {profile['churn_risk']}
            ...
            st.warning(
                "Please ensure all models are trained and saved correctly.")
